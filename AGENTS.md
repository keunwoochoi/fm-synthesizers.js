# fm-synthesizers.js — Agent Router
...
Sideband placement, Bessel amplitudes, tuning, stability, headroom and determinism all have
closed-form ground truth, so CI can block on numbers the physical project could only review by ear.
FM's alias problem is the crown jewel: the Bessel series is infinite, so the alias gate grades the
worst case over ratio × index × algorithm and holds oversampling honest. **An exact metric is not a
safe metric:** the DSP returns a buffer and the harness computes every number, the harness is not
editable by what it grades, gates use the **worst case** over the sweep rather than the average, and
the cheats are written first and proven to lose.
...
| Patch/preset ids have exactly one definition | guard test greps for the literal elsewhere |
...
| "Is this right?", evals, gates | `agentic-docs/design/2026-08-02-verification-and-harness.md` |
| Architecture, roster, phasing | `agentic-docs/design/2026-08-02-architecture.md` |
...
| The token cost of this build | `agentic-docs/build-usage-log.md` |

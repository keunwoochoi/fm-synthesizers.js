...
export async function createEngine({ wasmUrl, workletUrl, context, initialEvents, connect = true } = {}) {
  ...
  try {
    ...
    const wasm = wasmUrl ?? new URL("./wasm/fm_dsp.wasm", import.meta.url);
    const res = await fetch(wasm);
    if (!res.ok) {
      throw new Error(
        `fm-synthesizers: could not load WASM from ${wasm} (HTTP ${res.status}). ` +
    ...
    node = new AudioWorkletNode(ctx, "fm", {
      numberOfInputs: 0,
      numberOfOutputs: 1,
    ...
    const ready = realtime ? new Promise((resolve, reject) => {
      ...
      readyTimer = setTimeout(() => {
        readyState = "rejected";
        reject(new Error("fm-synthesizers: worklet never signalled ready"));
      }, 5000);
    }) : Promise.resolve();
    const disposedAck = new Promise((resolve) => { resolveDisposed = resolve; });
    const waitForDisposed = () => new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new Error("fm-synthesizers: worklet did not acknowledge disposal")),
        5000,
      );
      disposedAck.then(() => {
        clearTimeout(timer);
    ...
    const asError = (detail) => detail instanceof Error
      ? detail
      : new Error(`fm-synthesizers: ${String(detail)}`);
    const report = (detail) => {
    ...
    node.port.onmessage = (event) => {
      ...
      if (message?.type === "ready" && readyState === "pending") {
        ...
        report(new Error(`fm-synthesizers worklet: ${message.message}`));
    ...
    node.port.onmessageerror = () => report(new Error(
      "fm-synthesizers: worklet message failed to deserialize"));
    node.onprocessorerror = () => report(new Error(
      "fm-synthesizers: AudioWorklet processor crashed during construction/render"));
    ...
    let disposePromise;
    const assertLive = () => {
      if (disposed) throw new Error("fm-synthesizers: engine is disposed");
    };
    const resume = async () => {
      assertLive();

...
const P = { index: 1, feedback: 2, gain: 4, op2Sustain: 15, op3Sustain: 21,
            chorusRate: 30, chorusDepth: 31, chorusMix: 29, algorithm: 0 };
...
function buildReference() {
  ...
  // The expensive path is ON: high index (more sidebands = more operator work), the
  // feedback algorithm, and the full chorus send.
  x.set_param(e, P.algorithm, 3);
  x.set_param(e, P.index, 2.0);
  x.set_param(e, P.feedback, 0.6);
  x.set_param(e, P.op2Sustain, 0.85);
  x.set_param(e, P.op3Sustain, 0.85);
  x.set_param(e, P.chorusRate, 0.42);
  x.set_param(e, P.chorusDepth, 4.2);
  x.set_param(e, P.chorusMix, 0.85);
  // FILL THE POOL. FM voices are cheap per operator, but a 16-voice pool with 4
  // operators each at 4x oversampling is the honest worst case for this build.

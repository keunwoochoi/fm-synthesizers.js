...
#
# FM starts with the checker already in place: there is no pre-checker history, so
# there is no migration to grandfather and no preset is ever eligible for retrospective
# provenance. The empty cutoff makes that structural, not aspirational.
...
def _preset_ids_from_source(source: str, where: str) -> tuple[set[str], str | None]:
    ...
    bank = re.sub(r"\b(?:SHAPE|FILTER|ALGORITHM)\.\w+", "0", bank)
...
def _cutoff_ids(root: Path) -> tuple[set[str], str | None]:
    if not RETROSPECTIVE_CUTOFF:
        # No pre-checker history: nothing is eligible for retrospective provenance.
        return set(), None
...
def _first_preset_commits(root: Path) -> tuple[dict[str, str], str | None]:
    ...
    if run.returncode:
        # A repository with no commits yet (bootstrap, pre first commit) has no history
        # to inspect. The prior-provenance gate exists to prove ordering once the bank
        # is populated; an empty bank has nothing to order.
        if "unknown revision" in run.stderr or "bad revision" in run.stderr:
            return {}, None

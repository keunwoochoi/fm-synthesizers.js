...
const { instance } = await WebAssembly.instantiate(
  readFileSync("packages/core/wasm/fm_dsp.wasm"), {});
...
// not, because you just reach for the volume.
{
  ...
  if (lus.length) {
    ...
    console.log("  loudness: (bank empty at bootstrap; loudness gate is M3)");
  }
}
...
if (names.length >= 2) {
  ...
  console.log("  (bank empty at bootstrap; distinctness gate is M3)");
}

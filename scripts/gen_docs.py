...
def gen_product_summary() -> str:
    ...
    return (f"A {gen_bundle_total()} gzipped browser FM synthesizer with "
...
def gen_alias_table() -> str:
    """Measured alias suppression for an FM pair, at the shipped max index, across
    ratios. At low ratio no sideband approaches Nyquist and even the naive path is
    clean; the contrast that matters is where sidebands fold back. The M1 gate."""
    import numpy as np
    import candidates as C
    from metrics import measure_fm

    sr, dur = 48_000.0, 0.5
    n = int(dur * sr)
    fc, index = 440.0, 2.0
    rows = ["| ratio | naive FM | honest Bessel | shipped WASM |", "|---:|---:|---:|---:|"]
    for ratio in (1.0, 2.0, 3.0, 7.0):
        na = measure_fm(np.asarray(C.honest_fm_via_phase(fc, ratio, index, sr, n)),
                        fc, ratio, index, sr)["alias_db"]
        ho = measure_fm(np.asarray(C.honest_fm(fc, ratio, index, sr, n)),
                        fc, ratio, index, sr)["alias_db"]
        wa = measure_fm(np.asarray(C.wasm_fm(fc, ratio, index, sr, n)),
                        fc, ratio, index, sr)["alias_db"]
        rows.append(f"| {ratio} | {na:.1f} dB | {ho:.1f} dB | {wa:.1f} dB |")
    return "\n".join(rows) + "\n"
...
def gen_verdicts() -> str:
    ...
    rows = ["| candidate | alias dB | sideband err | verdict |", "|---|---:|---:|---|"]
    for name, fn in C.ALL.items():
        ...
        e = "—" if h["crashed"] else f"{h['sideband_err_db']:.1f}"
...
def gen_api() -> str:
    ...
    rows += ["",
             ...
             "`PARAM`, `ALGORITHM`, `RATIOS`, preset defaults, declarations, the playground, and the parameter table below derive from it."]
    return "\n".join(rows) + "\n"
